...
export class ReportsComponent implements AfterViewInit, OnDestroy {
  ...
  weeklyData = computed(() => {
    const allTxns = this.svc.transactions()
  ...
  categoryBreakdown = computed(() => {
    const txns = this.svc.transactions()
      .filter(t => t.type === 'expense' && new Date(t.date).getFullYear() === this.activeYear());
  ...
  summaryStats = computed(() => {
    const data = this.currentData().filter(d => d.income > 0 || d.expense > 0);
    ...
    const bestPeriod   = data.reduce((best, d) => d.net > best.net ? d : best,
      { label: '—', net: -Infinity, income: 0, expense: 0 });
    return { totalIncome, totalExpense, avgExpense, bestPeriod: bestPeriod.label, netSavings: totalIncome - totalExpense };
  });
  ...
  private buildTrend(): void {
    ...
    const c = new Chart(this.trendRef.nativeElement, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Income',   data: data.map(d => d.income),  backgroundColor: 'rgba(16,185,129,0.7)', borderRadius: 5, borderSkipped: false as const },
          { label: 'Expenses', data: data.map(d => d.expense), backgroundColor: 'rgba(239,68,68,0.7)',  borderRadius: 5, borderSkipped: false as const }
        ]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        scales: {
          x: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: '#7a84a0', font: { size: 11 } }, border: { color: 'rgba(255,255,255,0.07)' } },
          y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: '#7a84a0', callback: v => '$'+Number(v).toLocaleString() }, border: { color: 'rgba(255,255,255,0.07)' } }
        },
        plugins: {
          legend: { display: false },
          tooltip: { backgroundColor: '#1a1e2e', titleColor: '#e8eaf2', bodyColor: '#7a84a0', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1 }
        }
      }
    });
    this.charts.push(c);
  ...
  private buildCategoryBar(): void {
    ...
    const c = new Chart(this.catRef.nativeElement, {
      ...
      data: {
        ...
        datasets: [{ label: 'Amount', data: cats.map(c => c.amt), backgroundColor: cats.map(c => c.color), borderRadius: 5, borderSkipped: false as const }]
      },
      options: {
        indexAxis: 'y' as const,
        responsive: true, maintainAspectRatio: false,
        scales: {
          x: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: '#7a84a0', callback: v => '$'+Number(v).toLocaleString() }, border: { color: 'rgba(255,255,255,0.07)' } },
          y: { grid: { color: 'transparent' }, ticks: { color: '#7a84a0', font: { size: 11 } }, border: { color: 'rgba(255,255,255,0.07)' } }
        },
        plugins: { legend: { display: false }, tooltip: { backgroundColor: '#1a1e2e', titleColor: '#e8eaf2', bodyColor: '#7a84a0', borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1 } }
      }
    });
    this.charts.push(c);
  }
  ...
  exportCSV(): void  { this.expSvc.exportToCSV(this.svc.transactions(), `full-report-${this.activeYear()}`); }
  exportPDF(): void  { this.expSvc.exportToPDF(this.svc.transactions(), `Annual Report ${this.activeYear()}`); }
