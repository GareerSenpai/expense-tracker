  ...
  exportToPDF(transactions: Transaction[], title = 'Expense Report'): void {
    import('jspdf').then(({ jsPDF }) => {
      import('jspdf-autotable').then(({ default: autoTable }) => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.text('$' + totalExpense.toFixed(2), 101, 59, { align: 'center' });

        const net = totalIncome - totalExpense;
        doc.setFillColor(net >= 0 ? 230 : 252, net >= 0 ? 248 : 232, net >= 0 ? 240 : 232);
        doc.roundedRect(134, 46, 54, 16, 3, 3, 'F');
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(net >= 0 ? 10 : 139, net >= 0 ? 100 : 30, net >= 0 ? 60 : 30);
        ...
        autoTable(doc, {
          ...
          body: sorted.map(t => [
            t.date, t.title, t.category,
            t.type.charAt(0).toUpperCase() + t.type.slice(1),
            '$' + t.amount.toFixed(2),
            t.notes || ''
          ]),
          startY: 72,
          headStyles: { fillColor: [245, 158, 11], textColor: [15, 14, 20], fontStyle: 'bold' },
